...
// Define golbal vector of joints last position, moving state of the arm, and the client that can request services
...
void move_arm_center()
{
  ...
  // Request centerd joint angles [1.57, 1.57]
...
void joint_state_callback(const sensor::JointState js)
{
  ...
  if (fabs(joints_current_position[0] - joints_current_position[0]) < tolerance && fabs(joints_current_position[1] - joints_current_position[1]) < tolerance)
    moving_state = false;
  else {
    moving_state = true;
...
int main(int argc, char** argv){
  ...
  // Subscribe to /simple_arm/joint_states topic to read the arm joints position inside the joint_state_callback function
